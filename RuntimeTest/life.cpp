...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < gen; i++) {
      ...
      for (int i = 0; i < gen; i++) {
          ...
          if(coords[0]!=0){
              ...
              //
              MPI_Wait(&request_send_up, &status);
          }
          
          if(coords[0]!=dims[0]-1){
              MPI_Wait(&request_recv_down, &status);
              //
              MPI_Wait(&request_send_down, &status);
          }
          ...
          if(coords[0]!=0){
              ...
              //
              MPI_Wait(&request_send_ucorner, &status);
          }
          
          if(coords[0]!=dims[0]-1){
              MPI_Wait(&request_recv_lcornor, &status);
              //
              MPI_Wait(&request_send_lcornor, &status);
          }
  ...
  if (rank == 0) {
      std::cout <<"For the problem size: "<< m <<" times "<<n<<", "<<size
      <<" processors, the runtime is "<< time_elapsed << std::endl;
